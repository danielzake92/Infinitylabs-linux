/********************************************************************************
 *  FILENAME:  Bitarray.h  DEVELOPER: Daniel Zaken  TEAM: Blue                  * 
 *                                                                              *
 *  REVIEWER:  Daniel Benedik                                                   * 
 *                                                                              *
 *  PURPOSE:  LIB for Modifying Bit - Array                                     *
 *                                                                              *
 *******************************************************************************/
/*INCLUDES*/
#include <stdio.h>/*Required for standard input ouput*/
#include <assert.h>/*Required for assert function*/
#include <string.h>/*Required for strlen*/
#include <limits.h> /*Required for CHAR_BIT macro definition*/
#include <stdlib.h>/*Required for dynamic memory allocation functions*/
#include "bitarray.h"/*Required for our library functions*/


static int g_mask_count[16] =
{
	0, 1, 1, 2,
	1, 2, 2, 3,
	1, 2, 2, 3, 
	2, 3, 3, 4
};

static const size_t g_mask_mirror[6] =
{
	0xFFFFFFFF00000000, 0xFFFF0000FFFF0000,
    0xFF00FF00FF00FF00, 0xF0F0F0F0F0F0F0F0,
    0xCCCCCCCCCCCCCCCC, 0xAAAAAAAAAAAAAAAA
};	

static const unsigned char TableByteReverse[256] = 
{
	
};

/*************************************************************/
bitsarray_t BitsArrSetOn( bitsarray_t arr, size_t index )
{
	return ( arr | ( (bitsarray_t)1 << index ) );
}
/*************************************************************/
bitsarray_t BitsArrSetOff( bitsarray_t arr, size_t index )
{
	return ( arr ^ ( (bitsarray_t)1 << index ) );
}
/*************************************************************/
bitsarray_t BitsArrSetBit( bitsarray_t arr, size_t index, int set )
{
	if(set)
	{
		return ( arr | ( (bitsarray_t)1 << index ) );
	}
	return ( arr ^ ( (bitsarray_t)1 << index ) );
}
/*************************************************************/
int BitsArrGetBit( bitsarray_t arr, size_t index )
{	
	return ( !!( arr & ( (bitsarray_t)1 << index ) ) );
}
/*************************************************************/
bitsarray_t BitsArrSetAll( bitsarray_t arr )
{
	(void)arr;
	return ( ~(bitsarray_t)0 );
}
/*************************************************************/
bitsarray_t BitsArrClearAll(bitsarray_t bits)
{
	(void)bits;
	return 0;
}
/*************************************************************/
size_t BitsArrCountOn( bitsarray_t arr )
{
	int counter = 0;
	
	while ( arr )
	{
		++counter;
        arr &= (arr - 1);
	}
	return counter;
}
/*************************************************************/
size_t BitsArrLutCountOn( bitsarray_t arr )
{
	int counter = 0;
					
	while(arr)
	{
		counter += g_mask_count[arr & (bitsarray_t)0xF];
		arr >>= 4; 
	}
	
	return counter;	
}
/*************************************************************/
size_t BitsArrCountOff( bitsarray_t arr )
{
	return BitsArrCountOn(~arr);
}
/*************************************************************/
bitsarray_t BitsArrFlip( bitsarray_t arr, size_t index )
{
	return ( arr ^ ( (bitsarray_t)1 << index ) );
}
/*************************************************************/
char *BitsArrToString( bitsarray_t arr, char *str )
{	
	int digit_num = sizeof(size_t) * CHAR_BIT;
	assert(str);
	
	str += digit_num;
	*str = '\0';
	--str;
		
	while( digit_num > 0)
	{
		*str = (arr % 2) + '0';
		arr /= 2;
		--str;
		--digit_num;
	}
	++str;
	return str;
}
/*************************************************************/
bitsarray_t BitsArrLeftShift(bitsarray_t bits, size_t num)
{
	return ( bits << num );
}
/*************************************************************/
bitsarray_t BitsArrRightShift(bitsarray_t bits, size_t num)
{
	return ( bits >> num );
}
/*************************************************************/
bitsarray_t BitsArrMirror(bitsarray_t bits)
{
	int i = 0;
	int shift_num = sizeof(size_t) * 4;
	while( i < 6 )
	{
		bits = (bits & g_mask_mirror[i] ) >> shift_num | (bits & ~g_mask_mirror[i]) << shift_num;		
		shift_num /= 2;
		++i;
	}
	return bits;
}
/*************************************************************/
bitsarray_t BitsArrLutMirror(bitsarray_t bits)
{
	size_t num = 0;
	int bytes_num = sizeof(size_t) -1;
	
	while(bytes_num)
	{
		num |= TableByteReverse[(unsigned char)bits];
		num <<= CHAR_BIT;
		bits >>= CHAR_BIT;
		--bytes_num;
	}
	return num ;		
}

char *MallocToBitsArrayStr()
{
    return ((char*)malloc(sizeof(bitsarray_t)*CHAR_BIT + 1));
}


