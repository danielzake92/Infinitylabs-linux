...
#include "pq_heap.h"    /* p_queue_t and declarations of all its functions */
...
{
	p_queue_t *pq = PQHeapCreate(&CompareChar);
...
static int Test1(void)
{
	...
	*/
	...
	TEST("IsEmpty", PQHeapIsEmpty(pq), 1);
	TEST("Size", PQHeapSize(pq), 0);
	
	status = PQHeapEnqueue(pq, char_arr + 5);
	TEST("Enqueue success or failure", status, 0);
	TEST("IsEmpty", PQHeapIsEmpty(pq), 0);
	TEST("Size", PQHeapSize(pq), 1);
	
	status = PQHeapEnqueue(pq, char_arr + 3);
	TEST("Enqueue success or failure", status, 0);
	TEST("IsEmpty", PQHeapIsEmpty(pq), 0);
	TEST("Size", PQHeapSize(pq), 2);
	
	status = PQHeapEnqueue(pq, char_arr + 2);
	TEST("Enqueue success or failure", status, 0);
	TEST("IsEmpty", PQHeapIsEmpty(pq), 0);
	TEST("Size", PQHeapSize(pq), 3);
	
	status = PQHeapEnqueue(pq, char_arr + 4);
	TEST("Enqueue success or failure", status, 0);
	TEST("IsEmpty", PQHeapIsEmpty(pq), 0);
	TEST("Size", PQHeapSize(pq), 4);
	
	value = PQHeapPeek(pq);
	TESTC("Peek", *value, char_arr[2]); 
	TEST("Size", PQHeapSize(pq), 4);
	
	value = PQHeapDequeue(pq);
	...
	status = PQHeapEnqueue(pq, char_arr + 3);
	TEST("Enqueue success or failure", status, 0);
	TEST("Size", PQHeapSize(pq), 6);
	
	c = char_arr[3];
	to_erase = &c;
	value = PQHeapErase(pq, &IsSameChar, to_erase);
	TESTC("PQHeapErase", *value, *to_erase);
	TEST("Size", PQHeapSize(pq), 5);
...
static void TestDurability(void)
{
	...
	for (i = 0; i < NUM_ITERATIONS; ++i)
	{
		...
		que = PQHeapCreate(&CompareChar);
		
		while (size < MAX_SIZE)
		{
			if ((0 == size) || (rand() % 7) <= 3)
			{
				status = PQHeapEnqueue(que, char_arr + (rand() % arr_len));
				if (0 != status)
				{
					printf(RED "Enqueue Failed, at line %d, in iteration %lu, "
					       "when size of queue was %lu\n\n" DEF, __LINE__, i, size);
					PQHeapDestroy(que); que = NULL;
					return;
				}
				++size;
			}
